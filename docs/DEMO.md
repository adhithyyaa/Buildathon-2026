...
| *"What broke, and what did you do about it?"* | Five real incidents in [`POSTMORTEM.md`](../POSTMORTEM.md), each with the committed regression that pins the fix — the headline: **our best model scored 99.5% and had learned nothing** (label leakage); we found it, injected irreducible noise, and it dropped to an honest ~70%. Plus a **failure-*prevention*** layer: CI tests (artifact-locked numbers, confidence bands, A/A) that make shipping a false headline number structurally **impossible**, not just something we watch for. |

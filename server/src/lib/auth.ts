 ...
 * token from `Authorization: Bearer <t>` or `x-sentinel-token`, compared in constant time to
 * SENTINEL_ADMIN_TOKEN. If the token is unset the endpoints are OPEN (zero-config local demo) — with
...
export function requireToken(req: Request, res: Response, next: NextFunction): void {
  const expected = env.SENTINEL_ADMIN_TOKEN;
  if (!expected) {
    ...
    if (env.NODE_ENV !== 'development') {
      res.status(503).json({ error: 'operator_auth_unconfigured', message: 'set SENTINEL_ADMIN_TOKEN to enable operator endpoints' });
      return;
    }
    if (!warned) {
      logger.warn('auth.disabled', { note: 'SENTINEL_ADMIN_TOKEN unset — operator endpoints are open (development only)' });
      warned = true;
    }
    return next();
  }
  const header = req.header('authorization') ?? '';
  const provided = header.replace(/^Bearer\s+/i, '') || req.header('x-sentinel-token') || '';

...
export async function explainCase(c: CaseNarrateInput): Promise<{ text: string; source: 'llm' | 'template' }> {
  const system =
    "You are Recoup's analyst. In 2-3 short, plain sentences explain to a merchant operations user WHY the recovery system chose this action for this failed payment. Reference the actual numbers. No jargon, no markdown, no preamble.";
  const user = [
